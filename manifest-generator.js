...
function getFiles(relativePath = "", files) {
  ...
  for (let file of files) {
    ...
    if (
      ...
      !IGNORED_CHARACTERS.includes(file[0])
    ) {
      reading += 1;
  ...
  if (file.slice(-4).includes(".js")) {
    ...
    if (packets) {
      ...
      for (let packet of packets) {
        ...
        let versions = [];
        for (
          let majorPatchVersion = 60;
          majorPatchVersion <= 100;
          majorPatchVersion += 1
        ) {
          let v;
          try {
        ...
        for (let packetVer of versions) {
          ...
          if (manifest.defs[packet[0]]) {
            ...
            // change to list
              ...
              ["number", "string"].includes(typeof manifest.defs[packet[0]]) &&
            ...
            if (onlyHighestDefVer && Array.isArray(manifest.defs[packet[0]])) {
              ...
              if (typeof manifest.defs[packet[0]][index] == "string")
                index -= 1;
...
function jsonify(obj) {
  ...
  if (lists)
    for (let list of lists) {
      obj =
        obj.substring(0, obj.indexOf(list)) +
        list.replace(/[ \n\t]*/gim, "") +
        obj.substring(obj.indexOf(list) + list.length);
    }
  return obj;
}
